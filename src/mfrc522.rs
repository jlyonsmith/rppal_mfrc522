...
// The inverse calculation to get the prescale value
static PRESCALE: f64 = (MFRC_FREQ - TICK_FREQ) / (2.0 * TICK_FREQ);
// The MFRC522 countdown timer interval
...
impl Mfrc522<'_> {
    ...
    pub fn reset(&mut self) -> Result<()> {
        ...
        // TPrescaler_Hi=hh - high bits of prescaler value
        self.write(Register::TModeReg, 0x80 | prescale_bytes[1])?;
        ...
        self.write(Register::TPrescalerReg, prescale_bytes[0])?;
        ...
        self.write(Register::TReloadRegLow, timer_tick_bytes[0])?;
        self.write(Register::TReloadRegHigh, timer_tick_bytes[1])?;
    ...
    pub fn read_card_id(&mut self) -> Result<u32> {
        ...
        Ok(if read_bytes.len() == 4 {
            u32::from_be_bytes(read_bytes[0..4].try_into()?)
        } else {
            0
        })
    }
    ...
    fn transceive_with_card(&mut self, send_data: &[u8]) -> Result<(Vec<u8>, usize)> {
        ...
        self.write(Register::ComlEnReg, 0b1111011u8)?;
        ...
        self.write(Register::FIFOLevelReg, 0x80)?;
        ...
        self.write(Register::BitFramingReg, 0x80)?;
        ...
        loop {
            ...
            if (irq_bits & 0b0000_0001u8) != 0u8 {
                timed_out = true;
                break;
            }
            if (irq_bits & 0b0011_0000u8) != 0u8 {
                break;
            }
        ...
        // If we timed out
        if timed_out {
            return Err(Box::new(Mfrc522Error::Timeout));
        }

        self.write(Register::BitFramingReg, 0x80)?;

        if self.read(Register::ErrorReg)? & 0b0001_1011 != 0 {
            return Err(Box::new(Mfrc522Error::ReadingCard));
        }

        let num_fifo_bytes = self.read(Register::FIFOLevelReg)? as usize;
